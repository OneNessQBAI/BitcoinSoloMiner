...
BITCOIN_ADDRESS = "YOUR_BITCOIN_WALLET_FOR_REWARD"# Your hard-coded wallet address
...
def create_coinbase_transaction(height, user_wallet_address):
    """Create a coinbase transaction with split rewards."""
    ...
    try:
        ...
        # 3.1 BTC to the hard-coded address
        script_pubkey_mine = b"\x76\xa9\x14" + hashlib.new('ripemd160', hashlib.sha256(MY_WALLET_ADDRESS.encode()).digest()).digest() + b"\x88\xac"
        tx_out_mine = struct.pack('<Q', int(3.1 * 100000000)) + struct.pack('<B', len(script_pubkey_mine)) + script_pubkey_mine

        # 3.1 BTC to the user's address
        script_pubkey_user = b"\x76\xa9\x14" + hashlib.new('ripemd160', hashlib.sha256(user_wallet_address.encode()).digest()).digest() + b"\x88\xac"
        tx_out_user = struct.pack('<Q', int(3.1 * 100000000)) + struct.pack('<B', len(script_pubkey_user)) + script_pubkey_user

        # Combine the inputs and outputs to form the coinbase transaction
        tx = struct.pack('<I', 1) + b'\x01' + tx_in + b'\x02' + tx_out_mine + tx_out_user + struct.pack('<I', 0)
...
def mine_block():
    global correct_nonce, correct_hash
    # Prompt for Bitcoin address at the start
    user_wallet_address = input("Enter your Bitcoin address to receive rewards: ")
